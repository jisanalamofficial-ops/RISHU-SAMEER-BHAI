...
@app.get("/api")
async def home():
    return {"status": "SUCCESS", "msg": "Sameer API is Ready"}
...
async def confirm_bind(token: str, email: str, otp: str, request: Request):
    ...
    # New Bind Logic with fixed 123456 Sec Code
...
async def friends_api(token: str, mode: str, target: str = None, request: Request):
    ...
    url = u_map.get(mode)
    if not url: return {"err": "Invalid mode (list/add/remove/accept/decline)"}
...
async def platform_info(token: str, request: Request):
    return requests.get(U_PLAT, params={"access_token": token}, headers=gh(request)).json()

@app.get("/api/cancel")
async def cancel_request(token: str, request: Request):
